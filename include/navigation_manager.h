...
public:
    ...
    // State management
    void setPageCount(int pageCount) { m_state.pageCount = pageCount; }
    void setCurrentPage(int currentPage) { m_state.currentPage = currentPage; }
    void setLastRenderDuration(Uint32 duration) { m_state.lastRenderDuration = duration; }
    
    // State accessors
    int getCurrentPage() const { return m_state.currentPage; }
    int getPageCount() const { return m_state.pageCount; }
    bool isPageJumpInputActive() const { return m_state.pageJumpInputActive; }
    const std::string& getPageJumpBuffer() const { return m_state.pageJumpBuffer; }
