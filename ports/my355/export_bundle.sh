#!/usr/bin/env bash
# Export MY355 bundle - creates a complete distribution package
set -Eeuo pipefail
...
if [ -f "$PROJECT_ROOT/build/my355/sdl_reader_cli" ]; then
    echo "Copying sdl_reader_cli binary..."
    cp "$PROJECT_ROOT/build/my355/sdl_reader_cli" "$BUNDLE_DIR/bin/"
else
    echo "Warning: sdl_reader_cli not found in build/my355/ - did you run 'make my355'?"
fi

# Generate library bundle using make_bundle.sh
echo "Generating library dependencies..."
cd "$PROJECT_ROOT"
export BIN="./build/my355/sdl_reader_cli"
export DEST="./ports/my355/pak/lib"
...
echo ""
echo "MY355 bundle exported successfully to: $BUNDLE_DIR"
