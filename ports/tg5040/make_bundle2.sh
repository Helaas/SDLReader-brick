...
BINDIR="$DEST/bin"
LIBDIR="$DEST/lib"
EXCL_REGEX='(^ld-linux-|^libc\.so\.|^libpthread\.so\.|^libm\.so\.|^librt\.so\.|^libdl\.so\.|^libnsl\.so\.|^libresolv\.so\.)'
...
if command -v patchelf >/dev/null 2>&1; then
  patchelf --set-rpath '$ORIGIN/../lib' --force-rpath "$BINDIR/sdl_reader_cli"
  for so in "$LIBDIR"/*.so*; do
    patchelf --set-rpath '$ORIGIN' --force-rpath "$so" || true
  done
else
  echo "WARNING: patchelf not found; skipping RPATH embed."
fi

# Launcher with LD_LIBRARY_PATH as fallback
