...
# - Folders first, then PDFs, CBZ/CBR/ZIP files, EPUB books, and MOBI e-books (case-insensitive)
...
build_json_list() {
  ...
  for p in "$dir"/*; do
    ...
    if [ -d "$p" ]; then
      ...
      case "$p" in
        *.[Pp][Dd][Ff]|*.[Cc][Bb][Zz]|*.[Cc][Bb][Rr]|*.[Zz][Ii][Pp]|*.[Ee][Pp][Uu][Bb]|*.[Mm][Oo][Bb][Ii]) printf '%s\n' "$p" >>"$docs_tmp" ;;
      esac
    fi
  done
...
browse() {
  ...
  while :; do
    ...
    case "$sel_path" in
      *.[Pp][Dd][Ff]|*.[Cc][Bb][Zz]|*.[Cc][Bb][Rr]|*.[Zz][Ii][Pp]|*.[Ee][Pp][Uu][Bb]|*.[Mm][Oo][Bb][Ii])
