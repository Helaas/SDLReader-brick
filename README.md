...
#### MY355 (Embedded Linux - Miyoo Flip)
* Prebuilt cross-compilation toolchain container: `ghcr.io/loveretro/my355-toolchain`
* Docker environment available (see `ports/my355/` for build configuration)
* **Same control scheme** as TrimUI Smart Pro; keyboard scancode duplicates are filtered to prevent double-processing
* **Custom power management**: Dynamic input device discovery and direct sysfs suspend
...
### TrimUI & Miyoo Embedded Devices (TG5040, TG5050 & MY355)

The TG5040 (TrimUI Brick & Smart Pro), TG5050 (TrimUI Smart Pro S), and MY355 (Miyoo Flip) share the same feature set:
...
- **Bundle Export**: Complete distribution package creation
  ...
  - Ready for device deployment via `make export-tg5040`, `make export-tg5050`, `make export-my355`, or `make export-trimui`
...
└── ports/                        # Platform-specific builds
    ...
    │   ├── export_bundle.sh      # Bundle export script
    │   └── make_bundle.sh        # Library dependency bundler
    ├── my355/                    # MY355 embedded device (Miyoo Flip)
    │   ├── makefile              # MY355 build configuration
    │   ├── Dockerfile            # Thin wrapper for the GHCR toolchain image
    │   ├── export_bundle.sh      # Bundle export script
    │   ├── make_bundle.sh        # Library dependency bundler
    │   ├── include/              # MY355-specific headers
    │   │   └── power_handler.h   # Power management with dynamic device discovery
    │   └── src/                  # MY355-specific source code
    │       └── power_handler.cpp # Power button handling with EVIOCGBIT scan
    ├── mac/                      # macOS desktop
...
- **Power management**: TrimUI devices (TG5040/TG5050) share hardware-specific power button handling in `ports/tg5040/`; MY355 has its own power handler with dynamic device discovery in `ports/my355/`
...
- **TrimUI distribution**: Use `make export-tg5040`, `make export-tg5050`, or `make export-my355` to create complete deployment packages for a device, or `make export-trimui` to create a single package for all platforms
